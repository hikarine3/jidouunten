...
| 項目 | 値 | 確認方法 |
|---|---:|---|
| sitemap掲載URL数 | 246 | `dist/sitemap-0.xml` の `<loc>` 件数（sitemap index自身は除外） |
| うち車両詳細 | 241 | `src/data/vehicles.json` の販売単位レコード数 |
...
構成は「#17で公開済み販売単位を継続拡大」を前提とする。現時点の詳細ページは241件だが、未確認候補群を含む国内全候補の網羅率は未確定である。
...
|---|---|---|---|
| #17 販売単位の網羅 | S1の分母 | 車両詳細 241件を起点に、未掲載母集団と欠損理由を解決 | 掲載販売単位数・母集団カバー率 |
