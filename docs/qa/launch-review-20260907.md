...
- **安全表現・根拠:** Level 2は運転支援・常時監視、Level 3はODD内のシステム運転と引継ぎを表示（`src/data/loader.ts:188-193`, `src/pages/cars/[id].astro:12,17`）。分類根拠と「メーカー認証・安全保証ではない」注意を詳細へ表示（同:17）。車両ごとに発行元、URL、確認日、対象事実を表示（同:18）。
