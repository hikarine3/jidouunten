...
- 固定配信版の初回再検証では、比較遷移直後のchecked件数が0となるassertion失敗を検出。
  ...
  比較URLの確定と実際の比較結果visibleを待ってから件数判定するよう、テストの待機だけを修正。
  アプリ無変更で固定配信版の3/3回、ローカルの1/1回が全シナリオPASS。
  初回失敗は即時count()が画面初期化前に判定したタイミング競合と整合する。
  失敗を隠すretryやDOM値の書換えは追加していない。
  待機修正後に親も本体で再実行し1/1シナリオPASS、GA collect 204を確認。
