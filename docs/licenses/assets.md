...
確認日: 2026-09-11。車種識別用に、Wikimedia Commonsでファイル単位のライセンスを確認した参考写真を3モデルへ追加した。メーカーのロゴ素材・外部フォントは使用しない（写真内に偶然写る車体バッジはロゴ単体の転載とは分け、出所表示や提携を示す表現に使わない）。
...
人物・識別可能なナンバープレートを含む素材は、別途許諾と商標・肖像・プライバシー確認がない限り採用しない。
車体に固着したメーカー／車名バッジが写る場合は、ロゴ単体を配布せず、メーカーの推薦・提携を示さない車種識別目的に限定して確認する。
