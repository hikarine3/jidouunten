...
メーカーがモデル単位で「注文後の工場出荷時期目処」を公開している場合は、`sources[].supports`
へ記録し、一覧・詳細・比較に「工場出荷目処」として補助表示できる。これは個別販売単位の注文受付、
在庫、納車日を保証しないため、`availability` はグレード単位の一次情報がない限り `unknown` のままとする。
