...
try {
  ...
  await page.reload();
  assert.equal(await visibleCards(), 20, '既定カタログは現行確認20件');
  ...
  assert.equal(await visibleCards(), 20, '戻る後の結果件数');
  ...
  assert.equal(await visibleCards(), 20, 'リセット後に既定20件');
  ...
  assert.equal(await visibleCards(), 21, 'すべての状態で過去車両を含む21件');
