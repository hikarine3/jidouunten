...
try {
  ...
  assert.equal(rootItemList.numberOfItems, 411, 'トップItemListは初期可視411件');
  assert.equal(rootItemList.itemListElement.length, 411, 'トップItemList要素数は初期可視411件');
  assert.deepEqual(rootItemList.itemListElement.map(({ position }) => position), Array.from({ length: 411 }, (_, index) => index + 1), 'トップItemList positionを連番で出力');
  ...
  assert.match(await page.locator('.level-scope-note').innerText(), /Level 1[\s\S]*28件/, '車両一覧のLevel 1取り扱い件数を明記');
  ...
  assert.deepEqual(await page.locator('[data-status-shortcut]').allTextContents(), ['新車注文可21', '注文可否 未確認390', '現在利用不可1'], '販売状態の内訳を一覧の操作盤に表示');
  ...
  assert.equal(await visibleCards(), 390, '注文可否未確認は390販売単位');
  await page.locator('[data-reset-shortcut]').click();
  ...
  assert.match(await page.locator('meta[name="description"]').getAttribute('content'), /日本で選べる自動運転・Level 1\/2運転支援車411販売単位/, 'トップのdescription件数は公開データから生成');
  ...
  assert.equal(await levelMapPage.locator('[data-vehicle-shell]:not([hidden])').count(), 383, 'Level 2現行383件へ復帰');
  await levelMapPage.close();
  await page.goto(`${base}/levels/`);
  assert.match(await page.locator('.level-1').innerText(), /現行掲載 28件[\s\S]*このレベルの車両/, 'Level 1の現行掲載をレベル解説にも明記');
  ...
  assert.equal(await visibleCards(), 125, '本体価格の開始値が300万円未満の候補へ絞り込む');
  ...
  assert.equal(await visibleCards(), 411, '戻る後の結果件数');
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-vehicle-shell]:not([hidden])').length === 411);
  assert.equal(await visibleCards(), 411, 'リセット後に既定411件');
  ...
  assert.equal(await visibleCards(), 412, 'すべての状態で過去車両を含む412件');
