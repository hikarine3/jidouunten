...
try {
  ...
  await page.reload();
  assert.equal(await visibleCards(), 29, '既定カタログは現行確認29件');
  ...
  assert.equal(await visibleCards(), 29, '戻る後の結果件数');
  ...
  assert.equal(await visibleCards(), 29, 'リセット後に既定29件');
  ...
  assert.equal(await visibleCards(), 30, 'すべての状態で過去車両を含む30件');
