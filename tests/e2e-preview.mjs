...
try {
  ...
  await page.reload();
  assert.equal(await visibleCards(), 188, '既定カタログは現行確認188件');
  assert.match(await page.locator('.catalog-command').innerText(), /同じLevel 2でも[\s\S]*できることは違う[\s\S]*188[\s\S]*条件内可[\s\S]*48[\s\S]*不可[\s\S]*139[\s\S]*未確認[\s\S]*1[\s\S]*車線変更支援[\s\S]*50/, 'トップ操作盤に能力差の実データ分布');
  ...
  assert.match(await page.locator('meta[name="description"]').getAttribute('content'), /日本で選べる自動運転・Level 2運転支援車188販売単位/, 'トップのdescription件数は公開データから生成');
  ...
  assert.equal(await levelMapPage.locator('[data-vehicle-shell]:not([hidden])').count(), 188, 'Level 2現行188件へ復帰');
  ...
  assert.equal(await visibleCards(), 188, '戻る後の結果件数');
  ...
  assert.equal(await visibleCards(), 188, 'リセット後に既定188件');
  ...
  assert.equal(await visibleCards(), 189, 'すべての状態で過去車両を含む189件');
