...
try {
  ...
  assert.equal(rootItemList.numberOfItems, 304, 'トップItemListは初期可視304件');
  assert.equal(rootItemList.itemListElement.length, 304, 'トップItemList要素数は初期可視304件');
  assert.deepEqual(rootItemList.itemListElement.map(({ position }) => position), Array.from({ length: 304 }, (_, index) => index + 1), 'トップItemList positionを連番で出力');
  assert.equal(rootGraph.filter((entry) => entry['@type'] === 'BreadcrumbList').length, 1, 'トップBreadcrumbListは重複しない');
  await page.goto(`${base}/`);
  await page.evaluate(() => localStorage.clear());
  await page.reload();
  assert.equal(await visibleCards(), 304, '既定カタログは現行確認304件');
  assert.match(await page.locator('.catalog-command').innerText(), /同じLevel 2でも[\s\S]*できることは違う[\s\S]*304[\s\S]*条件内可[\s\S]*66[\s\S]*不可[\s\S]*237[\s\S]*未確認[\s\S]*1[\s\S]*車線変更支援[\s\S]*64/, 'トップ操作盤に能力差の実データ分布');
  ...
  assert.deepEqual(await page.locator('[data-status-shortcut]').allTextContents(), ['新車注文可21', '注文可否 未確認283', '現在利用不可1'], '販売状態の内訳を一覧の操作盤に表示');
  ...
  assert.equal(await visibleCards(), 283, '注文可否未確認は283販売単位');
  await page.locator('[data-reset-shortcut]').click();
  ...
  assert.match(await page.locator('meta[name="description"]').getAttribute('content'), /日本で選べる自動運転・Level 1\/2運転支援車304販売単位/, 'トップのdescription件数は公開データから生成');
  ...
  assert.equal(await levelMapPage.locator('[data-vehicle-shell]:not([hidden])').count(), 302, 'Level 2現行302件へ復帰');
  ...
  assert.equal(await visibleCards(), 304, '戻る後の結果件数');
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-vehicle-shell]:not([hidden])').length === 304);
  assert.equal(await visibleCards(), 304, 'リセット後に既定304件');
  ...
  assert.equal(await visibleCards(), 305, 'すべての状態で過去車両を含む305件');
