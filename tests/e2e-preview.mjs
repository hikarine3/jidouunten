...
try {
  ...
  await page.reload();
  assert.equal(await visibleCards(), 16, '既定カタログは現行確認16件');
  ...
  assert.equal(await visibleCards(), 16, '戻る後の結果件数');
  ...
  assert.equal(await visibleCards(), 16, 'リセット後に既定16件');
  ...
  assert.equal(await visibleCards(), 17, 'すべての状態で過去車両を含む17件');
