...
try {
  ...
  assert.equal(rootItemList.numberOfItems, 314, 'トップItemListは初期可視314件');
  assert.equal(rootItemList.itemListElement.length, 314, 'トップItemList要素数は初期可視314件');
  assert.deepEqual(rootItemList.itemListElement.map(({ position }) => position), Array.from({ length: 314 }, (_, index) => index + 1), 'トップItemList positionを連番で出力');
  assert.equal(rootGraph.filter((entry) => entry['@type'] === 'BreadcrumbList').length, 1, 'トップBreadcrumbListは重複しない');
  await page.goto(`${base}/`);
  await page.evaluate(() => localStorage.clear());
  await page.reload();
  assert.equal(await visibleCards(), 314, '既定カタログは現行確認314件');
  assert.match(await page.locator('.catalog-command').innerText(), /同じLevel 2でも[\s\S]*できることは違う[\s\S]*314[\s\S]*条件内可[\s\S]*66[\s\S]*不可[\s\S]*247[\s\S]*未確認[\s\S]*1[\s\S]*車線変更支援[\s\S]*64/, 'トップ操作盤に能力差の実データ分布');
  ...
  assert.deepEqual(await page.locator('[data-status-shortcut]').allTextContents(), ['新車注文可21', '注文可否 未確認293', '現在利用不可1'], '販売状態の内訳を一覧の操作盤に表示');
  ...
  assert.equal(await visibleCards(), 293, '注文可否未確認は293販売単位');
  await page.locator('[data-reset-shortcut]').click();
  ...
  assert.equal(await visibleCards(), 64, '車線変更支援は64件');
  await page.locator('[data-reset-shortcut]').click();
  ...
  assert.match(await page.locator('meta[name="description"]').getAttribute('content'), /日本で選べる自動運転・Level 1\/2運転支援車314販売単位/, 'トップのdescription件数は公開データから生成');
  ...
  assert.equal(await levelMapPage.locator('[data-vehicle-shell]:not([hidden])').count(), 312, 'Level 2現行312件へ復帰');
  ...
  assert.equal(await visibleCards(), 57, 'トップのLevel 2・高速・ハンズオフ条件は57件（フォレスター5グレードを含む）');
  ...
  assert.equal(await visibleCards(), 61, '能力チェックは複数選択をAND条件で適用');
  ...
  assert.equal(await visibleCards(), 61, '保存したAND条件の再開結果');
  await page.locator('[data-saved-resume-delete="search"]').click();
  ...
  assert.equal(await visibleCards(), 52, '使い方ガイドが高速道路・渋滞・条件内ハンズオフへ絞り込む');
  ...
  assert.equal(await visibleCards(), 57, 'Level 2・高速・ハンズオフ条件は57件（フォレスター5グレードを含む）');
  ...
  assert.equal(await visibleCards(), 314, '戻る後の結果件数');
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-vehicle-shell]:not([hidden])').length === 314);
  assert.equal(await visibleCards(), 314, 'リセット後に既定314件');
  ...
  assert.equal(await visibleCards(), 315, 'すべての状態で過去車両を含む315件');
