...
try {
  ...
  assert.equal(rootItemList.numberOfItems, 413, 'トップItemListは初期可視413件');
  assert.equal(rootItemList.itemListElement.length, 413, 'トップItemList要素数は初期可視413件');
  assert.deepEqual(rootItemList.itemListElement.map(({ position }) => position), Array.from({ length: 413 }, (_, index) => index + 1), 'トップItemList positionを連番で出力');
  assert.equal(rootGraph.filter((entry) => entry['@type'] === 'BreadcrumbList').length, 1, 'トップBreadcrumbListは重複しない');
  await page.goto(`${base}/`);
  await page.evaluate(() => localStorage.clear());
  await page.reload();
  assert.equal(await visibleCards(), 413, '既定カタログは現行確認413件');
  ...
  assert.match(await page.locator('.catalog-command').innerText(), /同じLevel 2でも[\s\S]*できることは違う[\s\S]*413[\s\S]*条件内可[\s\S]*70[\s\S]*不可[\s\S]*342[\s\S]*未確認[\s\S]*1[\s\S]*車線変更支援[\s\S]*68/, 'トップ操作盤に能力差の実データ分布');
  ...
  assert.match(await page.locator('meta[name="description"]').getAttribute('content'), /日本で選べる自動運転・Level 1\/2運転支援車413販売単位/, 'トップのdescription件数は公開データから生成');
  ...
  assert.equal(await levelMapPage.locator('[data-vehicle-shell]:not([hidden])').count(), 385, 'Level 2現行385件へ復帰');
  ...
  assert.equal(await visibleCards(), 413, '戻る後の結果件数');
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-vehicle-shell]:not([hidden])').length === 413);
  assert.equal(await visibleCards(), 413, 'リセット後に既定413件');
  ...
  assert.equal(await visibleCards(), 414, 'すべての状態で過去車両を含む414件');
