...
try {
  ...
  assert.equal(rootItemList.numberOfItems, 395, 'トップItemListは初期可視395件');
  assert.equal(rootItemList.itemListElement.length, 395, 'トップItemList要素数は初期可視395件');
  assert.deepEqual(rootItemList.itemListElement.map(({ position }) => position), Array.from({ length: 395 }, (_, index) => index + 1), 'トップItemList positionを連番で出力');
  assert.equal(rootGraph.filter((entry) => entry['@type'] === 'BreadcrumbList').length, 1, 'トップBreadcrumbListは重複しない');
  await page.goto(`${base}/`);
  await page.evaluate(() => localStorage.clear());
  await page.reload();
  assert.equal(await visibleCards(), 395, '既定カタログは現行確認395件');
  assert.match(await page.locator('.catalog-command').innerText(), /同じLevel 2でも[\s\S]*できることは違う[\s\S]*395[\s\S]*条件内可[\s\S]*68[\s\S]*不可[\s\S]*326[\s\S]*未確認[\s\S]*1[\s\S]*車線変更支援[\s\S]*66/, 'トップ操作盤に能力差の実データ分布');
  assert.equal(await page.locator('[data-level-shortcut]').count(), 5, 'Level 1〜5を同時表示');
  assert.match(await page.locator('[data-level-shortcut="1"]').innerText(), /L1[\s\S]*20件/, 'Level 1の現行20件を表示');
  assert.equal(await page.locator('[data-level-shortcut="1"]').isDisabled(), false, '現行車があるLevel 1を絞り込み可能にする');
  assert.match(await page.locator('.level-scope-note').innerText(), /Level 1[\s\S]*20件/, '車両一覧のLevel 1取り扱い件数を明記');
  ...
  assert.deepEqual(await page.locator('[data-status-shortcut]').allTextContents(), ['新車注文可21', '注文可否 未確認374', '現在利用不可1'], '販売状態の内訳を一覧の操作盤に表示');
  ...
  assert.equal(await visibleCards(), 374, '注文可否未確認は374販売単位');
  await page.locator('[data-reset-shortcut]').click();
  ...
  assert.match(await page.locator('meta[name="description"]').getAttribute('content'), /日本で選べる自動運転・Level 1\/2運転支援車395販売単位/, 'トップのdescription件数は公開データから生成');
  ...
  assert.equal(await levelMapPage.locator('[data-vehicle-shell]:not([hidden])').count(), 375, 'Level 2現行375件へ復帰');
  await levelMapPage.close();
  await page.goto(`${base}/levels/`);
  assert.match(await page.locator('.level-1').innerText(), /現行掲載 20件[\s\S]*このレベルの車両/, 'Level 1の現行掲載をレベル解説にも明記');
  ...
  assert.equal(await visibleCards(), 109, '本体価格の開始値が300万円未満の候補へ絞り込む');
  ...
  assert.equal(await visibleCards(), 395, '戻る後の結果件数');
  ...
  await page.waitForFunction(() => document.querySelectorAll('[data-vehicle-shell]:not([hidden])').length === 395);
  assert.equal(await visibleCards(), 395, 'リセット後に既定395件');
  ...
  assert.equal(await visibleCards(), 396, 'すべての状態で過去車両を含む396件');
