...
describe('vehicle data contract and filters', () => {
  ...
  it('日産キックスはP16現行12販売単位を価格・プロパイロット能力付きで保持する', () => {
    const kicks = vehicles.filter((vehicle) => vehicle.maker === 'Nissan' && vehicle.model === 'キックス');
    expect(kicks).toHaveLength(12);
    expect(kicks.map((vehicle) => vehicle.price?.amounts[0].amountJpy).sort((a, b) => (a ?? 0) - (b ?? 0))).toEqual([
      2_999_700, 3_259_300, 3_349_500, 3_485_900, 3_549_700, 3_599_200, 3_880_800, 3_898_400, 3_899_500, 3_996_300, 4_248_200, 4_309_800,
    ]);
    expect(kicks.every((vehicle) => vehicle.currentCatalogListed && vehicle.generation === 'P16' && vehicle.catalogAsOf === '2026-06' && vehicle.salesUnitIntroducedAt === '2026-06' && vehicle.priceEffectiveAt === '2026-06')).toBe(true);
    expect(kicks.every((vehicle) => vehicle.automationLevel === 2 && vehicle.category === 'driver_assistance' && vehicle.driverMonitoring === 'required' && vehicle.handsOff === 'not_allowed' && vehicle.availability === 'unknown')).toBe(true);
    expect(kicks.every((vehicle) => vehicle.capabilities.join(',') === 'adaptive_cruise_control,lane_centering')).toBe(true);
    expect(kicks.every((vehicle) => vehicle.odd.speedKph.min === 30 && vehicle.odd.speedKph.max === 135)).toBe(true);
    expect(kicks.every((vehicle) => vehicle.sources.some((source) => source.url.endsWith('/kicks/specifications.html') && source.supports.some((fact) => fact.includes('円'))))).toBe(true);
    expect(kicks.every((vehicle) => vehicle.sources.some((source) => source.url.includes('faq2.nissan.co.jp/faq/show/82222')))).toBe(true);
    expect(officialLinkFor(kicks[0])?.actions?.map(({ kind }) => kind)).toEqual(['dealer', 'test_drive', 'estimate', 'catalog']);
  });
  ...
  it('トヨタ GRヤリスは2026年3月の全10販売単位を価格・LTA差付きで保持する', () => {
    const grYaris = vehicles.filter((vehicle) => vehicle.model === 'GRヤリス');
    expect(grYaris).toHaveLength(10);
    expect(grYaris.map((vehicle) => vehicle.price?.amounts[0].amountJpy).sort((a, b) => (a ?? 0) - (b ?? 0))).toEqual([
      3_617_200, 3_967_200, 4_112_200, 4_462_200, 4_537_200, 4_887_200, 5_037_200, 5_387_200, 5_532_200, 5_882_200,
    ]);
    expect(grYaris.every((vehicle) => vehicle.currentCatalogListed && vehicle.catalogAsOf === '2026-03' && vehicle.priceEffectiveAt === '2026-03' && vehicle.salesUnitIntroducedAt === null && vehicle.automationLevel === 2 && vehicle.driverMonitoring === 'required' && vehicle.handsOff === 'not_allowed' && vehicle.availability === 'unknown')).toBe(true);
    expect(grYaris.every((vehicle) => vehicle.capabilities.join(',') === 'adaptive_cruise_control,lane_centering')).toBe(true);
    expect(grYaris.every((vehicle) => vehicle.odd.speedKph.min === null && vehicle.odd.speedKph.max === null)).toBe(true);
    expect(grYaris.every((vehicle) => vehicle.sources.some((source) => source.url.endsWith('grades60.json') && source.supports.some((fact) => fact.includes('円'))))).toBe(true);
  ...
  it('ダイハツ タント系はグレード・駆動方式・スマートクルーズパック差を別単位で保持する', () => {
    const tanto = vehicles.filter((vehicle) => vehicle.maker === 'Daihatsu');
    expect(tanto).toHaveLength(20);
    expect(tanto.map((vehicle) => vehicle.price?.amounts[0].amountJpy).sort((a, b) => (a ?? 0) - (b ?? 0))).toEqual([
      1_496_000, 1_622_500, 1_628_000, 1_628_000, 1_749_000, 1_749_000, 1_793_000, 1_820_500, 1_820_500, 1_881_000,
      1_881_000, 1_914_000, 1_941_500, 1_941_500, 1_963_500, 2_002_000, 2_002_000, 2_024_000, 2_084_500, 2_145_000,
    ]);
    expect(tanto.every((vehicle) => vehicle.currentCatalogListed && vehicle.catalogAsOf === '2026-09' && vehicle.salesUnitIntroducedAt === '2024-10' && vehicle.priceEffectiveAt === null && vehicle.automationLevel >= 1 && vehicle.driverMonitoring === 'required' && vehicle.handsOff === 'not_allowed' && vehicle.availability === 'unknown')).toBe(true);
    expect(tanto.filter((vehicle) => vehicle.automationLevel === 1)).toHaveLength(8);
    expect(tanto.filter((vehicle) => vehicle.automationLevel === 2)).toHaveLength(12);
    expect(tanto.filter((vehicle) => vehicle.price?.optionalPackages.length).length).toBe(6);
    expect(tanto.filter((vehicle) => vehicle.capabilities.includes('lane_departure_prevention'))).toHaveLength(20);
    expect(tanto.filter((vehicle) => vehicle.capabilities.includes('adaptive_cruise_control'))).toHaveLength(12);
    expect(tanto.every((vehicle) => vehicle.sources.some((source) => source.url.includes('daihatsu.co.jp/lineup/tanto')))).toBe(true);
    expect(officialLinkFor(tanto.find((vehicle) => vehicle.model === 'タント')!)?.actions?.map(({ kind }) => kind)).toEqual(['dealer', 'test_drive', 'estimate', 'catalog']);
    expect(officialLinkFor(tanto.find((vehicle) => vehicle.model === 'タント ファンクロス')!)?.actions?.map(({ kind }) => kind)).toEqual(['dealer', 'test_drive', 'estimate', 'catalog']);
  });
  ...
  it('Land Rover Discovery Sportは3グレードを価格・ACC/LKA付きで保守的に保持する', () => {
    const discovery = vehicles.filter((vehicle) => vehicle.maker === 'Land Rover' && vehicle.model === 'Discovery Sport');
    expect(discovery).toHaveLength(3);
    expect(discovery.map((vehicle) => vehicle.grade).sort()).toEqual(['Dynamic S', 'Landmark', 'Metropolitan'].sort());
    expect(discovery.map((vehicle) => vehicle.price?.amounts[0].amountJpy).sort((a, b) => (a ?? 0) - (b ?? 0))).toEqual([7_240_000, 8_230_000, 10_710_000]);
    expect(discovery.every((vehicle) => vehicle.currentCatalogListed && vehicle.availability === 'unknown' && vehicle.modelYear === null && vehicle.salesUnitIntroducedAt === null && vehicle.priceEffectiveAt === null && vehicle.automationLevel === 2 && vehicle.handsOff === 'not_allowed' && vehicle.driverMonitoring === 'required')).toBe(true);
    expect(discovery.every((vehicle) => vehicle.price?.kind === 'range' && vehicle.price.maxJpy === null && vehicle.capabilities.join(',') === 'adaptive_cruise_control,lane_centering')).toBe(true);
    expect(discovery.every((vehicle) => vehicle.odd.roadTypes.length === 0 && vehicle.odd.speedKph.min === null && vehicle.odd.speedKph.max === null && vehicle.sources.some((source) => source.url === 'https://www.landrover.co.jp/discovery/discovery-sport/models-and-specifications.html'))).toBe(true);
    expect(discovery.every((vehicle) => !vehicle.capabilities.includes('lane_change_support'))).toBe(true);
    expect(officialLinkFor(discovery[0])?.actions?.map(({ kind }) => kind)).toEqual(['estimate', 'test_drive', 'dealer']);
  });
