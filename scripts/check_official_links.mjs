...
const actionCount = links.reduce((count, link) => count + (link.actions?.length || 0), 0);
console.log(`PASS: 公式導線 ${links.length}モデル / ${uniqueUrls.size} URL / 次アクション${actionCount}件 / 全${vehicles.length}販売単位 / 現行${current.length}件product`);
